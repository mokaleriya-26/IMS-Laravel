...
<div class="flex min-h-screen bg-[#F8FAFC]">
    <aside class="w-64 bg-white border-r border-slate-200 flex flex-col fixed h-full z-20 shadow-sm">
        <div class="p-6 flex-1">
            <div class="flex items-center gap-3 mb-8">
                <div class="bg-amber-600 text-white p-2 rounded-xl shadow-md">
                ...
                <div>
                    <h3 class="font-extrabold text-slate-900 text-[14px] truncate">{{ $branch ?: 'Branch Admin' }}</h3>
                    <p class="text-[11px] text-slate-500">Branch Portal</p>
                </div>
            </div>
            <nav class="space-y-1">
                <a href="{{ route('branch.admin.dashboard') }}" class="flex items-center gap-3 px-4 py-3 text-sm font-bold rounded-xl bg-amber-50 text-amber-700">
                ...
                <a href="{{ route('branch.admin.students') }}" class="flex items-center gap-3 px-4 py-3 text-sm font-bold rounded-xl text-slate-600 hover:bg-slate-50">
                ...
                <a href="{{ route('branch.admin.faculty') }}" class="flex items-center gap-3 px-4 py-3 text-sm font-bold rounded-xl text-slate-600 hover:bg-slate-50">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857"/></svg>
                    Faculty
                </a>
                <a href="{{ route('branch.admin.requests') }}" class="flex items-center gap-3 px-4 py-3 text-sm font-bold rounded-xl text-slate-600 hover:bg-slate-50">
                ...
                <a href="{{ route('branch.admin.notices') }}" class="flex items-center gap-3 px-4 py-3 text-sm font-bold rounded-xl text-slate-600 hover:bg-slate-50">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z"/></svg>
                    Branch Notices
                </a>
                <a href="{{ route('branch.admin.reports') }}" class="flex items-center gap-3 px-4 py-3 text-sm font-bold rounded-xl text-slate-600 hover:bg-slate-50">
    ...
    <div class="pl-64 flex-1">
        <header class="h-16 bg-white border-b border-slate-200 flex items-center px-8 sticky top-0 z-10 shadow-sm">
            <h1 class="text-lg font-black text-slate-800">{{ $branch }} — Branch Dashboard</h1>
        </header>
        <main class="p-8 space-y-6">
            ...
            {{-- Hero Banner --}}
            <div class="bg-amber-600 rounded-3xl p-8 text-white relative overflow-hidden shadow-xl shadow-amber-600/10">
                <h2 class="text-3xl font-extrabold tracking-tight">{{ $branch }} Portal</h2>
                <p class="text-amber-100 mt-1 text-sm">Manage student and faculty listings, publish notices, and view branch statistics.</p>
            </div>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                <div class="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    ...
                    <a href="{{ route('branch.admin.notices') }}" class="text-sm font-bold text-amber-600 hover:underline">Manage Notices</a>
                </div>
