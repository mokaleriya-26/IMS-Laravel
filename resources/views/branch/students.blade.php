...
<div class="flex min-h-screen bg-[#F8FAFC]">
    <aside class="w-64 bg-white border-r border-slate-200 flex flex-col fixed h-full z-20 shadow-sm p-6">
        ...
        <nav class="space-y-1">
            ...
            <a href="{{ route('branch.admin.students') }}" class="block px-4 py-3 text-sm font-bold rounded-xl bg-amber-50 text-amber-700">Students</a>
    ...
    <div class="pl-64 flex-1">
        <header class="h-16 bg-white border-b border-slate-200 flex items-center px-8 sticky top-0 z-10 shadow-sm">
            <h1 class="text-lg font-black text-slate-800">Branch Students – {{ $branch }}</h1>
        </header>
        <main class="p-8">
            <form method="GET" class="flex flex-wrap gap-3 mb-6">
                ...
                <button type="submit" class="bg-amber-600 text-white font-bold text-sm px-5 py-2.5 rounded-xl hover:bg-amber-700 transition">Filter</button>
